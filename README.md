...
### Bugs
The site had two interesting bugs, the nav bar not correctly displaying on mobile devices and the buttons on the route suggestion section wrapping. I managed to fix the nav bar by aligning it to the left on all devices. The button not displaying correctly was also fixed by changing it to one word, which is not ideal but did the job.
